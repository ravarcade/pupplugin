...
// Call from swap loop.   Prepares textures once the sizes are detected by the 
// capture thread. 
...
void captureCheckTextures()
{
 /*  if (ecDMDStage == ecTexture)
   ...
   {
      ecPUP.m_pData = PuPVideo::CreateTexture(ecPUP.m_Width, ecPUP.m_Height);
...
void captureStartup()
{
//   ecDMDStage = ecSearching : ecFailure;
   ecPUPStage = ecSearching; // : ecFailure;
   StopCapture = false;
